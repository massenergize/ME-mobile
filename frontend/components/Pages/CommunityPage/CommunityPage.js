...
function GoalsCard({ navigation, goals }) {
  const goalsList = [
    {
      ...
      current:
        goals.initial_number_of_actions + goals.attained_number_of_actions,
    },
    {
      ...
      current:
        goals.initial_number_of_households +
    ...
    },
    {
      ...
      current:
        goals.initial_carbon_footprint_reduction +
  ...
  return (
    <Pressable
      onPress={() => navigation.navigate("impact", { goalsList: goalsList })}
      mx={4}
    >
...
export default function CommunityPage({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    //     console.log(json.data.subdomain);
    //   } else {
    //     console.log(json);
    //   }
    //   setIsLoading(false);
          ...
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
            <HStack space={2} justifyContent="center" mx={15} marginBottom={15}>
              {actions.data.map((action, index) => {
                if (
                  getMetric(action, "Cost") === "$" ||
                  getMetric(action, "Cost") === "0"
                ) {
                  return (
                    <ActionCard
                      navigation={navigation}
